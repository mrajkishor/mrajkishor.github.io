...
const App = () => {
  ...
  useEffect(() => {
    ...
    // that depends on the current route (location.pathname). 
    // This ensures the parent app is notified every time 
    // the route changes inside the iframe.

    // Why this?
    // - The iframe is a separate browsing context. 
    //   Its internal navigation (using React Router or similar) 
    //   only updates the URL of the iframe, not the parent window.
    // - The parent app’s URL will not change unless I 
    //   explicitly synchronize navigation events between the 
    //   iframe and the parent.

    window.parent.postMessage(
      { type: "NAVIGATE", path: location.pathname },
      "*"
    );

    // Its listener is at (vite + react) app
    // mrajkishor.github.io\app\src\App.tsx
  }, [location.pathname]);
