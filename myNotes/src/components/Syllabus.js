...
import q_bank from "../assets/qbank.jpg";
...
import fs_integration from "../assets/fs_inte.webp";
...
const Tile = ({ title, link, image, progress, onClick }) => (
    <a
        // href={link}
        onClick={onClick}
        style={{
...
const renderTiles = (topics, basePath) => {
    ...
    const placeholderImages = {
        ...
        "Full Stack Integration": fs_integration,
    ...
    return Object.entries(topics)
        ...
        .map(([topic, value], idx) => {
            ...
            const handleClick = () => {
                if (topic === "Q Bank") {
                    const password = prompt("Enter the password to access Q Bank:");
                    if (btoa(password) !== "aW50ZXJ2aWV3QHBybw==") {
                        alert("Incorrect password. Access denied.");
                        return;
                    }
                }
                window.location.href = fullPath; // navigate only after password is correct
            };
    ...
    return (
        <FadeInOnView>
            ...
            <div className="syllabus-footer">
                <i> Topic Wise Concepts, Use Cases, Case Studies, STAR Method, Code Samples, CIQnA</i>
            </div >
        </FadeInOnView >
    );
};
